...
import { json, useLoaderData, useFetcher,useNavigate, useLocation } from "@remix-run/react";
...
function Addonlist() {
   ...
   useEffect(() => {
    // If the `reload` signal is present, fetch the data again
    if (state?.reload) {
        // window.location.reload();
        // location.reload(); 
    }
   }, [state]); // Run this effect when the `state` changes, which happens when navigating
   ...
   return (
       <Page title="Product Addons"
            secondaryActions={
                ...
                <InlineStack align="center"> <Icon source={PlusIcon} tone="base" /> Create Addon </InlineStack>
                </Button>
            }
           >
           {rows.length === 0 && (
